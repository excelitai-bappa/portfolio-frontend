...
export default {
  ...
  methods: {
    async getTestimonial() {
      ...
      await axios
        .get(`http://127.0.0.1:8000/api/testimonial/show/${this.id}`)
        .then((response) => {
    ...
    updateTestimonial() {
      ...
      axios
        .post(
          `http://127.0.0.1:8000/api/testimonial/update/${this.id}`,
          formData,
          {
        ...
        .then(function () {
          Swal.fire({
            icon: "success",
            title: "Testimonial Infromation Updated Successfully",
          });
